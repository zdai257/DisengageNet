...
class GazeDataset(torch.utils.data.dataset.Dataset):
    def __init__(self, dataset_name, path, split, transform, in_frame_only=True, sample_rate=1, aug_groups=None, depth_dir='depth'):
    ...
    def __getitem__(self, idx):
        ...
        # load depth_map as .npy
        depth_map = self._load_depth_npy(img_data["path"])
        ...
        if self.split == "train":  # note for training set, there is only one annotation
            heatmap = utils.get_depthaware_heatmap(depth_map, gazex_norm[0], gazey_norm[0], 64, 64)
