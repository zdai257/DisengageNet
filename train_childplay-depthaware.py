"""
train_childplay-depthaware.py — depth-aware ChildPlay finetuning (GazeMoE + aux 3D).

Finetunes a GazeFollow-pretrained GazeMoE on ChildPlay-gaze with the same two
depth-aware changes as train_gazefollow-depthaware.py:
...
Everything else follows train_childplay.py (CSV annotations, native-resolution
resize, per-bucket LR incl. inout_lr, VAT-style AUC/L2 eval, in/out AP).
"""
...
class ChildPlayDepthDataset(torch.utils.data.Dataset):
    """ChildPlay-gaze with depth-aware heatmap target + DA2 depth map."""

    def __init__(self, dir_path, split, transform, aug_groups=None,
                 depth_dir="depth"):
        ...
        for clip in os.listdir(join(dir_path, "annotations", split)):
            ...
            for _, frame in df.iterrows():
                ...
                rel_path = join("images", clip.split(".")[0], img_idx)
                ...
                self.data.append((rel_path, bbox, gazex, gazey, inout,
                                  res.iloc[0]))

        num_in = sum(item[4] for item in self.data)
        print(f"{split}: {len(self.data)} samples  "
              f"({num_in} in-frame, {len(self.data) - num_in} out-of-frame, "
    ...
    def __getitem__(self, idx):
        rel_path, bbox, gazex, gazey, inout, res = self.data[idx]
        gazex, gazey = [gazex], [gazey]
        inout = int(inout)

        img_path = join(self.dir_path, rel_path)
        ...
        depth_pil = self._load_depth_pil(rel_path, img.size)
        width, height = img.size

        xmin, ymin, xmax, ymax = bbox
        bbox = [max(xmin, 0), max(ymin, 0),
...
def main():
    ...
    wandb.init(project=cfg_m["name"], name="childplay_depth_aux", config=config)
    ...
    cp_path = cfg_d.get("childplay_path", "./ChildPlay-gaze")
    depth_dir = cfg_d.get("depth_dir", "depth")

    train_dataset = ChildPlayDepthDataset(
        cp_path, "train", transform,
        aug_groups=cfg_d.get("augmentations", []), depth_dir=depth_dir)
    eval_dataset = ChildPlayDepthDataset(
        cp_path, "test", transform, depth_dir=depth_dir)
    ...
    w_inout = float(cfg_m.get("bce_weight", 0.1)) or 0.1

    print(f"Auxiliary loss: gaze3d_aux  w_aux_3d={w_aux_3d_target}  "
          f"warmup={aux_warmup_eps} epoch(s)  w_inout={w_inout}")
    ...
    for epoch in range(epochs):
        ...
        for cur_iter, batch in tqdm(enumerate(train_dl), total=n_iters,
            ...
            loss = l_hm + w_inout * l_inout + w_aux_cur * l_aux
            loss.backward()
            if grad_clip > 0:
